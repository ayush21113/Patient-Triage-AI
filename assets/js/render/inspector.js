...
function renderDerivation(assessment, protocol) {
 ...
 const information = assessment.derivation.information ?? [];
 const informationShift = information.reduce((sum, item) =>
  sum + item.shift, 0
 );
 section.append(derivationLine(
  information.length === 0 ? "Priority Index" : "Model index",
  `100 × (${weights.physiology}×L1 + ${weights.presentation}×L2 + ${weights.hazard}×L3) ÷ ${maximum}`,
  number(assessment.derivation.modelIndex ??
   assessment.priorityIndex - informationShift)
 ));
 for (const item of information) {
  section.append(derivationLine(
   "Question answer",
   `${item.questionId} · ${item.answer.replace("_", " ")}`,
   `${item.shift >= 0 ? "+" : ""}${item.shift}`
  ));
 }
 if (information.length > 0) {
  section.append(derivationLine(
   "Priority Index",
   "Model index + recorded information",
...
export function renderInspector(
 ...
 const question = renderQuestion(
  assessment,
  onQuestionAnswer && (answer => onQuestionAnswer(selected, answer))
 );
